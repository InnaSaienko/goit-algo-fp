...
def draw_tree(x, y, length, angle, depth, max_depth):
    ...
    color = (depth / max_depth, 0.3, 1 - depth / max_depth)

    plt.plot([x, x_end], [y, y_end], color=color, linewidth=2)
    new_length = length * 0.7
    draw_tree(x_end, y_end, new_length, angle + np.pi / 6, depth - 1, max_depth)
    draw_tree(x_end, y_end, new_length, angle - np.pi / 6, depth - 1, max_depth)
